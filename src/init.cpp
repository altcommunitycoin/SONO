...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: SONO.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: SONOd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 29855 or testnet: 39855)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 28855 or testnet: 38855)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeSONOamount=<n> " + _("Keep N SONO anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SONO is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SONO version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    nAnonymizeSONOAmount = GetArg("-anonymizeSONOamount", 0);
    if(nAnonymizeSONOAmount > 999999) nAnonymizeSONOAmount = 999999;
    if(nAnonymizeSONOAmount < 2) nAnonymizeSONOAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeSONOAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize SONO Amount %d\n", nAnonymizeSONOAmount);

    /* Denominations
       A note about convertability. Within luxsend pools, each denomination
       is convertable to another.
       For example:
       1SONO+1000 == (.1SONO+100)*10
       10SONO+10000 == (1SONO+1000)*10
    */
