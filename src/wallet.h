...
enum AvailableCoinsType
{
    ...
    ONLY_NONDENOMINATED_NOTMN = 4 // ONLY_NONDENOMINATED and not 1000 SONO at the same time
};
...
private:
    ...
    bool SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_COINS, bool useIX = false) const;
...
public:
    ...
    //Used with luxsend. Will return fees, then denominations, everything else, then very small inputs that aren't fees
    int Priority() const
    {
        if(tx->vout[i].nValue == DARKSEND_FEE) return -1000;
        BOOST_FOREACH(int64_t d, darkSendDenominations)
            if(tx->vout[i].nValue == d) return 900;
