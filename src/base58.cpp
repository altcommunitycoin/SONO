...
namespace {
    class CSONOAddressVisitor : public boost::static_visitor<bool> {
    private:
        CSONOAddress *addr;
    public:
        CSONOAddressVisitor(CSONOAddress *addrIn) : addr(addrIn) { }
...
bool CSONOAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CSONOAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CSONOAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CSONOAddressVisitor(this), dest);
}

bool CSONOAddress::IsValid() const {
...
CTxDestination CSONOAddress::Get() const {
    if (!IsValid())
...
bool CSONOAddress::GetKeyID(CKeyID &keyID) const {
...
bool CSONOAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CSONOSecret::SetKey(const CKey& vchSecret) {
...
CKey CSONOSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CSONOSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CSONOSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CSONOSecret::SetString(const std::string& strSecret) {
