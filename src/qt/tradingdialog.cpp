    ...
    model(0)
{
    ...
    ui->SONOAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaSONOLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "SONO(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "SONO(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(SONO)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //SONOst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://cryptopia.co.nz/api/GetMarket/SONO_BTC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://cryptopia.co.nz/api/GetMarketOrders/SONO_BTC");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://cryptopia.co.nz/api/GetMarketHistory/SONO_BTC");
      return Response;
}
...
QString tradingDialog::BuySONO(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://cryptopia.co.nz";
            ...
            URL += "/api/SubmitTrade";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SONO&quantity=";
...
QString tradingDialog::SellSONO(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://cryptopia.co.nz";
            URL += OrderType;
            URL += "/api/GetMarket/SONO_BTC";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SONO&quantity=";
            URL += str.number(Quantity,'i',8);
            URL += "&rate=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://cryptopia.co.nz/api/GetOpenOrders";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SONO";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://cryptopia.co.nz/api/GetDepositAddress";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=SONO";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://cryptopia.co.nz/api/GetTradeHistory";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-SONO&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>SONO Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> SONO");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->SONOSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(SONOSupply,'i',8) + "</span><b> SONO</b>");
    ...
    ui->SONODemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(SONODemand,'i',8) + "</span><b> SONO</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("SONO");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->SONOAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("SONO");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->CryptopiaSONOLabel, *ui->CryptopiaBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("SONO");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->SONOBalanceLabel,*ui->SONOAvailableLabel_2,*ui->SONOPendingLabel, QString::fromUtf8("SONO"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputSONO->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("SONO");
    ...
    double AvailableSONO = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling SONO available balance
    QString responseA = GetBalance("SONO");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableSONO = ResultObject["Available"].toDouble();

    ui->UnitsInputSONO->setText(str.number(AvailableSONO,'i',8));
}
...
{
    double Quantity = ui->CryptopiaSONOLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableSONO,'i',8));
}
...
void tradingDialog::on_BuySONO_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuySONO(Order,Quantity,Rate);
...
void tradingDialog::on_SellSONOBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputSONO->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellSONO(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellSONO(Order,y,x);
                ...
                QString SellResponse = SellSONO(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" SONO for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" SONO for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputSONO_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
