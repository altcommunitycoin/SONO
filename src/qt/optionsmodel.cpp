...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSONOAmount"))
        settings.setValue("nAnonymizeSONOAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSONOAmount = settings.value("nAnonymizeSONOAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSONOAmount"))
        SoftSetArg("-anonymizeSONOamount", settings.value("nAnonymizeSONOAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case luxsendRounds:
            ...
            break;
        case anonymizeSONOAmount:
            nAnonymizeSONOAmount = value.toInt();
            settings.setValue("nAnonymizeSONOAmount", nAnonymizeSONOAmount);
            emit anonymizeSONOAmountChanged(nAnonymizeSONOAmount);
            break;
        default:
            break;
        }
